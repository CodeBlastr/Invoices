...
<div class="invoice form"> <?php echo $this->Form->create('Invoice');?>
	<h3>Edit Invoice</h3>
	<?php echo $this->Form->input('Invoice.id'); ?>
	...
	<?php echo $this->Form->input('Invoice.due_date', array('value' => $dueDate, 'type' => 'datepicker')); ?>
  	...
  	<table>
    	<tbody class="invoiceItemLines">
    		...
    		<?php for ($i = 0; $i < count($this->request->data['InvoiceItem']); $i++) : ?>
      		<tr class="calc-row">
		        <td class="col-xs-3">
		        	<?php echo $this->Form->input('InvoiceItem.' . $i . '.id', array('type' => 'hidden', 'value' => $this->request->data['InvoiceItem'][$i]['id'])); ?>
		        	<?php echo $this->Form->input('InvoiceItem.' . $i . '.name', array('value' => $this->request->data['InvoiceItem'][$i]['name'], 'label' => false)); ?>
		        </td>
		        <td><?php echo $this->Form->input('InvoiceItem.' . $i . '.description', array('type' => 'text', 'label' => false, 'value' => $this->request->data['InvoiceItem'][$i]['description'])); ?></td>
		        <td><?php echo $this->Form->input('InvoiceItem.' . $i . '.unit_cost', array('label' => false, 'value' => $this->request->data['InvoiceItem'][$i]['unit_cost'], 'class' => 'calc-rate')); ?></td>
		        <td><?php echo $this->Form->input('InvoiceItem.' . $i . '.quantity', array('label' => false, 'value' => $this->request->data['InvoiceItem'][$i]['quantity'], 'class' => 'calc-multiplier')); ?></td>
		        <td class="lineTotal"> <?php echo $this->Form->input('InvoiceItem.' . $i . '.line_total', array('value' => '0.00', 'disabled' => 'disabled', 'label' => false, 'class' => 'calc-line-total text-right')); ?> </td>
	 		</tr>
			<?php endfor; ?>
			
		    <?php // this should always be an exact copy of the add page duplicatable with the 0 replaced ?>
    		<!-- duplicatable start -->
      		<tr class="duplicatable calc-row">
		        <td class="col-xs-3">
		        	<?php echo $this->Form->input('InvoiceItem.' . $i . '.name', array('empty' => '-- Select --', 'options' => array_merge($invoiceItems, array('+'=>'+ Custom Item')), 'div' => false, 'label' => false, 'class' => 'combobox')); ?>
		        </td>
		        <td><?php echo $this->Form->input('InvoiceItem.' . $i . '.description', array('type' => 'text', 'label' => false)); ?></td>
		        <td><?php echo $this->Form->input('InvoiceItem.' . $i . '.unit_cost', array('label' => false, 'class' => 'calc-rate')); ?></td>
		        <td><?php echo $this->Form->input('InvoiceItem.' . $i . '.quantity', array('label' => false, 'class' => 'changer calc-multiplier')); ?></td>
		        <td class="lineTotal"> <?php echo $this->Form->input('InvoiceItem.' . $i . '.line_total', array('value' => '0.00', 'disabled' => 'disabled', 'label' => false, 'class' => 'calc-line-total text-right')); ?> </td>
	 		</tr>
	 		<!-- duplicatable end -->
